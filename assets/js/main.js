...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (colaborador && colaborador.toLowerCase() === "lucas") {
    document
      .querySelectorAll("h1.h4, h1.h4.m-0")
      .forEach((el) => (el.textContent = "Lucas (Colaborador)"));
    document
      .querySelectorAll(".badge-level")
      .forEach((el) => (el.textContent = "Ouro"));
    document.querySelectorAll(".text-muted.small").forEach((el) => {
      if (el.textContent.includes("Loja"))
        el.textContent = "Loja Swift – Vila Mariana";
    });
    document
      .querySelectorAll(".progress-bar")
      .forEach((el) => (el.style.width = "65%"));
    document
      .querySelectorAll(".d-flex.justify-content-between.mt-1 small")
      .forEach((el, i) => {
    ...
    document.querySelectorAll(".ranking-badges").forEach((el) => {
      el.innerHTML =
        '<span class="badge bg-light text-dark" title="Ranking local">1º local</span> <span class="badge bg-light text-dark" title="Ranking global">2º global</span>';
    });
...
window.addEventListener("DOMContentLoaded", () => {
  try {
    ...
    if (user && user.role !== "gerente") {
      const managerLinks = Array.from(
        document.querySelectorAll("a.nav-link")
      ).filter(
        (a) => a.textContent && a.textContent.trim() === "Área do Gerente"
      );
...
function registrarVenda() {
  ...
  const base = Math.round(valor / 10);
  const pontos = cross ? base * 2 : base;
  alert(
    `Venda registrada. Pontos: +${pontos}${cross ? " (2× cross-sell)" : ""}`
  );
